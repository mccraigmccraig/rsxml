...
module Rsxml
    describe "find_namespace_uri" do
      it "should find a namespace uri in a stack of prefix bindings" do
        Rsxml::Namespace.find_namespace_uri([{"foo"=>"http://foo.com/foo"},
    ...
    describe "split_qname" do
      it "should split a name with prefix" do
        Rsxml::Namespace.split_qname("foo:bar").should == ["bar", "foo"]
      end

      it "should leave a name without prefix unchanged" do
        Rsxml::Namespace.split_qname("bar").should == "bar"
      end

      it "should leave an array unchanged" do
        Rsxml::Namespace.split_qname(["bar", "foo"]).should == ["bar", "foo"]
      end
    end

    describe "compact_attr_qnames" do
      it "should qualify attribute names when there is a default namespaces" do
        Rsxml::Namespace.compact_attr_qnames([{""=>"http://default.com/default","foo"=>"http://foo.com/foo"}], {["bar", "foo"] => "barbar", ["boo", ""] => "booboo", "baz" =>"bazbaz"}).should ==
          {"foo:bar"=>"barbar", "boo"=>"booboo", "baz"=>"bazbaz"}
      end

      it "should raise an exception if a namespace is referenced but not bound" do
        lambda {
          Rsxml::Namespace.compact_attr_qnames([], {["bar", "foo"] => "barbar", "baz" =>"bazbaz"})
        }.should raise_error(/not bound/)
      end

      it "should raise an exception if default namespace referenced but not bound" do
        lambda {
          Rsxml::Namespace.compact_attr_qnames([{"foo"=>"http://foo.com/foo"}], {["bar", "foo"] => "barbar", ["boo", ""] => "booboo", "baz" =>"bazbaz"})
        }.should raise_error(/not bound/)
      end
    end
    ...
    describe "explode_attr_qnames" do
      it "should unqualify attributes when there is no default namespace" do
        Rsxml::Namespace.explode_attr_qnames([{"foo"=>"http://foo.com/foo"}], {"foo:bar"=>"barbar", "baz"=>"bazbaz"}).should ==
          {["bar", "foo", "http://foo.com/foo"]=>"barbar", "baz"=>"bazbaz"}
      end

      it "should unqualify attributes when there is a default namespace" do
        Rsxml::Namespace.explode_attr_qnames([{""=>"http://default.com/default", "foo"=>"http://foo.com/foo"}], {"foo:bar"=>"barbar", "baz"=>"bazbaz"}).should ==
          {["bar", "foo", "http://foo.com/foo"]=>"barbar", "baz"=>"bazbaz"}
      end
    end
    ...
    describe "compact_qname" do
      it "should produce a qname from a pair or triple" do
        Rsxml::Namespace.compact_qname([{"foo"=>"http://foo.com/foo"}], 
                                 ["bar", "foo"]).should ==
          "foo:bar"
        Rsxml::Namespace.compact_qname([{"foo"=>"http://foo.com/foo"}], 
      ...
      it "should produce a qname with default namespace" do
        Rsxml::Namespace.compact_qname([{""=>"http://foo.com/foo"}], 
                                 ["bar", ""]).should ==
          "bar"
      ...
      it "should do nothing to a String" do
        Rsxml::Namespace.compact_qname([], "foo:bar").should == "foo:bar"
      end

      it "should raise an error if prefix is nil but uri is not" do
        lambda {
          Rsxml::Namespace.compact_qname([], ["bar", nil, "http://foo.com/foo"])
        }.should raise_error(/invalid name/)
      end

      it "should raise an error if a prefix is not bound" do
        lambda {
          Rsxml::Namespace.compact_qname([], 
                                   ["bar", "foo", "http://foo.com/foo"])
        }.should raise_error(/not bound/)
      end
    ...
    describe "extract_declared_namespace_bindings" do
      it "should extract a hash of declared namespace bindings from a Hash of attributes" do
        Rsxml::Namespace.extract_declared_namespace_bindings({"xmlns"=>"http://default.com/default", "xmlns:foo"=>"http://foo.com/foo", "foo"=>"bar"}).should ==
    ...
    describe "extract_explicit_namespace_bindings" do
      it "should extract a hash of explicit namespace bindings from expanded tags" do
        Rsxml::Namespace.extract_explicit_namespace_bindings(["bar", "foo", "http://foo.com/foo"],
    ...
    describe "undeclared_namespaces" do
      it "should determine which explicit namespaces are not declared in context" do
        Rsxml::Namespace.undeclared_namespaces([{""=>"http://default.com/default", "foo"=>"http://foo.com/foo"}], {""=>"http://default.com/default", "foo"=>"http://foo.com/foo", "bar"=>"http://bar.com/bar"}).should ==
          {"bar"=>"http://bar.com/bar"}
      end

      it "should raise an exception if an explicit prefix clashes with a declard prefix" do
        lambda {
          Rsxml::Namespace.undeclared_namespaces([{""=>"http://default.com/default", "foo"=>"http://foo.com/foo"}], {""=>"http://foo.com/foo", "foo"=>"http://foo.com/foo", "bar"=>"http://bar.com/bar"})
    ...
    describe "merge_namespace_bindings" do
      it "should merge two sets of namespace bindings" do
        Rsxml::Namespace.merge_namespace_bindings({""=>"http://default.com/default",
    ...
    describe "unqualified_namespace_declarations" do
      it "should produce unqalified namespaces declarations" do
        Rsxml::Namespace.unqualified_namespace_declarations({""=>"http://default.com/default", "foo"=>"http://foo.com/foo"}).should == 
